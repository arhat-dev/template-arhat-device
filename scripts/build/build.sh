...
template_arhat_device_go() {
  # TODO: set mandatory tags and predefined tags for specific platforms
  _build "${GOBUILD} -tags='nokube nocloud netgo ${PREDEFINED_BUILD_TAGS} ${TAGS}' ./cmd/template-arhat-device-go"
}
...
GO_LDFLAGS="-s -w \
  -X arhat.dev/template-arhat-device-go/pkg/version.branch=${GIT_BRANCH} \
  -X arhat.dev/template-arhat-device-go/pkg/version.commit=${GIT_COMMIT} \
  -X arhat.dev/template-arhat-device-go/pkg/version.tag=${GIT_TAG} \
  -X arhat.dev/template-arhat-device-go/pkg/version.arch=${ARCH} \
  -X arhat.dev/template-arhat-device-go/pkg/version.goCompilerPlatform=$(go version | cut -d\  -f4)"
